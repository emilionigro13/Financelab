...
Next UpDay 12 — Financial Ratios Calculator. Goal: Build an Analysis Engine that transforms raw fundamental data from Finnhub into 12 structured financial ratios, each enriched with formulas, descriptions, and health signals, and add a tab-based UI to the stock detail page separating market data from fundamental analysis. What was done: On the backend, we extended finnhub.service.ts with getBasicFinancials() calling the /stock/metric?metric=all endpoint with a 3600-second TTL cache since fundamental data changes quarterly rather than by the second, then we created analysis.engine.ts as the core business logic layer mapping raw Finnhub metric keys like peBasicExclExtraTTM, roeRfy, and currentRatioQuarterly into a typed AnalysisResult containing 12 FinancialRatio objects grouped into five categories: Valuation, Profitability, Leverage, Liquidity, and Dividends, where each ratio carries a raw value, a formatted human-readable string, a mathematical formula, an investor-oriented description, a category, and a signal evaluated through dual functions getSignal() for higher-is-better metrics and getInverseSignal() for lower-is-better metrics using standard industry thresholds, and we added the GET /api/v1/market/analysis/:symbol endpoint to market.routes.ts. On the frontend, we created RatioCard.tsx as a self-contained card showing the ratio name, category badge, formatted value, colored signal badge, and progressive disclosure on click to reveal formula and description, then we created FinancialRatios.tsx as a container fetching the analysis endpoint and rendering ratios grouped by category in a responsive grid, and we refactored the stock detail page to introduce Overview and Financial Analysis tabs moving existing price stats and charts under Overview and rendering FinancialRatios under Analysis. Issues encountered and resolved: Some Finnhub metric keys return undefined for non-US companies or recent IPOs with insufficient TTM data such as pegRatio or dividendYieldIndicatedAnnual, so instead of crashing the engine defaults to null and the frontend renders N/A with an unknown signal badge, which required careful null-coalescing to avoid NaN propagation into formattedValue. Concepts learned: Financially, we learned the difference between absolute valuation and growth-adjusted valuation via PEG, why ROE and ROA must be read together to detect leverage effects, that Quick Ratio is a stricter liquidity test than Current Ratio because it excludes inventory, and that sector context is critical since a P/E of 25 is expensive for a utility but normal for SaaS though the current engine uses universal thresholds for now. From a computer science perspective, we applied the Data Transformation Layer pattern separating the raw API adapter from domain logic, the Progressive Disclosure UX pattern to reduce cognitive load, cache stratification with different TTLs for market versus fundamental data, and an inverse threshold evaluation function to avoid duplicating signal logic across ratios. Commit: feat: add financial ratios analysis engine with 12 metrics. Next Up: Day 13 — Income Statement and Balance Sheet Viewer, where we will fetch full financial statements from Finnhub and build interactive data tables showing raw line items like Revenue, COGS, Operating Income, Net Income, Total Assets, and Total Liabilities across multiple periods to expose the underlying data behind today's ratios.